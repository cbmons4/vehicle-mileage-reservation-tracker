...
Public Sub FleetAddVehicle()
    ...
    branch = PromptRequired("Branch", "DEMO")
...
Public Sub FleetSaveDashboardMileage(Optional ByVal showMessage As Boolean = True)
    ...
    SetDebugStatus "Mileage: vehicle"
    ...
    SetDebugStatus "Mileage: add row"
    Set lr = lo.ListRows.Add
    ...
    SetDebugStatus "Mileage: refresh status"
    FleetRefreshMileageLogVehicleStatus False

    SetDebugStatus "Mileage: sync month"
    UpdateMonthlyMileageEntry logStartDate, vehicleId
    SetDebugStatus "Mileage: clear form"
    FleetClearMileageForm False
    SetDebugStatus "Mileage: refresh dashboard"
    FleetRefreshDashboard False
...
CleanFail:
    ...
    errorStage = CStr(ThisWorkbook.Worksheets("Setup").Range("B12").value)
    On Error GoTo 0
    If showMessage Or Application.Visible Then
        MsgBox "Could not log mileage (" & errorStage & "): " & errorDescription, vbCritical, APP_TITLE
    Else
...
Public Sub FleetSaveDashboardCheckout(Optional ByVal showMessage As Boolean = True)
    ...
    Do While reserveCurrentDate <= reserveEndDate
        conflictText = ReservationConflictText(lo, vehicleId, reserveCurrentDate)
    ...
    Do While reserveCurrentDate <= reserveEndDate
        Set lr = lo.ListRows.Add
        SetRowValue lo, lr, "Reservation ID", NextNumber(lo, "Reservation ID")
...
Public Sub FleetMarkVehicleInShop()
    On Error GoTo CleanFail
    ...
    If Not IsAllowedShopLocation(shopLocation) Then
        MsgBox "Choose Shop - Central Garage, Shop - North Service Center, or Shop - South Service Center.", vbExclamation, APP_TITLE
...
Public Sub FleetMarkVehicleAvailable()
    On Error GoTo CleanFail
    ...
    If IsDate(shopStart) Then
        AddShopReservationHistory vehicleId, DateOnly(shopStart), DateAdd("d", -1, Date), shopLocation
    End If
    ...
    MarkWeeklyReservationsDirty
    FleetRefreshMileageLogVehicleStatus False
    If IsDate(shopStart) Then RefreshMonthlyMileageShopRange DateOnly(shopStart), Date

    FleetRefreshDashboard False
    dashWs.Range("A29").value = "Shop status saved: " & VehicleChoiceTextForId(vehicleTable, vehicleId) & " is available."
    MsgBox "Vehicle marked available.", vbInformation, APP_TITLE
    Exit Sub
...
Public Sub FleetOpenWorkbook()
    ...
    ApplyPortfolioWorkbookSettings
...
Private Sub ApplyPortfolioWorkbookSettings()
    On Error Resume Next
    ...
    ThisWorkbook.BuiltinDocumentProperties("Company").value = "Professional Portfolio"
    ThisWorkbook.BuiltinDocumentProperties("Category").value = "Excel VBA Portfolio Project"
    ThisWorkbook.BuiltinDocumentProperties("Keywords").value = "Excel; VBA; fleet; mileage; reservations; dashboard; automation"
    ThisWorkbook.BuiltinDocumentProperties("Content Status").value = "Portfolio Demonstration"
    ThisWorkbook.BuiltinDocumentProperties("Comments").value = _
        "Portfolio demonstration developed by Charles Monsanto. " & _
        "Contains synthetic data only; no agency records or confidential information."
...
Private Function WorkbookIsCloudHosted() As Boolean
    ...
    WorkbookIsCloudHosted = (InStr(1, locationText, "https://", vbTextCompare) > 0 Or _
        InStr(1, locationText, "sharepoint", vbTextCompare) > 0 Or _
...
Private Sub EnsureDashboardUtilityButtons()
    ...
    AddOrUpdateDashboardUtilityButton ws, "btnUndoLastMileage", "C30:E30", "UNDO LAST MILEAGE", "FleetUndoLastMileage", RGB(238, 240, 242)
    AddOrUpdateDashboardUtilityButton ws, "btnUndoLastCheckout", "F30:H30", "UNDO LAST CHECKOUT", "FleetUndoLastCheckout", RGB(238, 240, 242)
End Sub
...
Private Sub UpdateDashboardData()
    ...
    Dim totalDrivingHours As Double
    ...
    SetDebugStatus "Dashboard: selectors"
    ...
    SetDebugStatus "Dashboard: selector values"
    ...
    SetDebugStatus "Dashboard: validation"
    ...
    SetDebugStatus "Dashboard: clear summary"
    ...
    For vehicleIndex = 1 To vehicleTable.ListRows.Count
        ...
        drivingHours = DrivingHoursForVehicleMonth(reservationTable, vehicleId, monthStart, monthEnd)
        ...
        WriteVehicleMonthlyRow dashWs, rowNumber, _
            ...
            lastOdometer, statusText, monthMiles, tripDays, tripCount, drivingHours, cumulativeMiles
    ...
    SetDebugStatus "Dashboard: totals"
    WriteVehicleMonthlyRow dashWs, 24, "TOTAL", "", "", totalMiles, totalTripDays, totalTrips, totalDrivingHours, totalCumulativeMiles
...
Private Sub ApplyMonthValidation(ByVal targetCell As Range, ByVal listText As String)
    ...
    On Error Resume Next
    Set validationCell = targetCell.MergeArea.Cells(1, 1)
    validationCell.Validation.Delete

    If Len(listText) > 0 Then
        validationCell.Validation.Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Operator:=xlBetween, Formula1:=listText
        validationCell.Validation.IgnoreBlank = False
        validationCell.Validation.InCellDropdown = True
    End If
    On Error GoTo 0
End Sub
...
Private Sub UpdateDashboardChoiceLists(ByVal dataWs As Worksheet, ByVal vehicleTable As ListObject, ByVal fiscalYearList As String, ByVal monthList As String)
    ...
    dataWs.Range("N3").value = "Shop - North Service Center"
    dataWs.Range("N4").value = "Shop - South Service Center"
...
Public Sub FleetRefreshMileageLogVehicleStatus(Optional ByVal showMessage As Boolean = False)
    ...
    If Not TableHasColumn(mileageTable, "Current Vehicle Status") Then
        Set statusColumn = mileageTable.ListColumns.Add
        statusColumn.Name = "Current Vehicle Status"
        needsWrite = True
    Else
    ...
    If Not mileageTable.DataBodyRange Is Nothing Then
        Set statusByVehicle = CreateObject("Scripting.Dictionary")
        statusByVehicle.CompareMode = vbTextCompare
        For rowIndex = 1 To vehicleTable.ListRows.Count
            vehicleId = Trim$(CStr(TableValue(vehicleTable, rowIndex, "Vehicle ID")))
            vehicleStatus = Trim$(CStr(TableValue(vehicleTable, rowIndex, "Status")))
            If StrComp(vehicleStatus, "Maintenance", vbTextCompare) = 0 Then
                statusText = ShopStatusText(vehicleTable, vehicleId)
            Else
                statusText = "Available"
            End If
            statusByVehicle(vehicleId) = statusText
        Next rowIndex
        ...
        For rowIndex = 1 To mileageTable.ListRows.Count
            ...
            If statusByVehicle.Exists(vehicleId) Then
                statusValues(rowIndex, 1) = statusByVehicle(vehicleId)
            Else
                statusValues(rowIndex, 1) = vbNullString
            End If
...
Private Function ReservationConflictText(ByVal reservationTable As ListObject, ByVal vehicleId As String, ByVal reserveDate As Date) As String
    Dim i As Long
                ...
                If DateOnly(rowDate) = DateOnly(reserveDate) Then
                    ...
                    If StrComp(statusText, "Cancelled", vbTextCompare) <> 0 Then
                        If IsShopReservationStatus(statusText) Then
                            ReservationConflictText = Format(DateOnly(rowDate), "m/d/yyyy") & " - " & ShopReservationLabel(statusText, _
...
Private Function IsAllowedShopLocation(ByVal shopLocation As String) As Boolean
    Select Case UCase$(Trim$(shopLocation))
        Case "SHOP - CENTRAL GARAGE", "SHOP - NORTH SERVICE CENTER", "SHOP - SOUTH SERVICE CENTER"
...
Private Sub AddShopReservationHistory(ByVal vehicleId As String, ByVal startDate As Date, ByVal endDate As Date, ByVal shopLocation As String)
    Dim reservationTable As ListObject
    Dim lr As ListRow
    Dim currentDate As Date
    ...
    Do While currentDate <= endDate
        If Not ShopReservationExists(reservationTable, vehicleId, currentDate, shopLocation) Then
            Set lr = reservationTable.ListRows.Add
            SetRowValue reservationTable, lr, "Reservation ID", NextNumber(reservationTable, "Reservation ID")
...
Private Function ShopReservationExists(ByVal reservationTable As ListObject, ByVal vehicleId As String, ByVal targetDate As Date, ByVal shopLocation As String) As Boolean
    Dim rowIndex As Long
    Dim rowDate As Variant
    Dim statusText As String
    ...
    For rowIndex = 1 To reservationTable.ListRows.Count
        If StrComp(Trim$(CStr(TableValue(reservationTable, rowIndex, "Vehicle ID"))), vehicleId, vbTextCompare) = 0 Then
            rowDate = TableValue(reservationTable, rowIndex, "Date")
            If IsDate(rowDate) Then
                If DateOnly(rowDate) = DateOnly(targetDate) Then
                    statusText = Trim$(CStr(TableValue(reservationTable, rowIndex, "Status")))
                    If IsShopReservationStatus(statusText) Then
                        If StrComp(statusText, shopLocation, vbTextCompare) = 0 Or shopLocation = "" Then
                            ShopReservationExists = True
                            Exit Function
                        End If
                    End If
                End If
...
Private Sub WriteSummaryHeaders(ByVal headerRange As Range)
    headerRange.value = Array("Vehicle", "Last Mileage", "Checkout Status (This Week)", "Month Miles", "Trip Days", "Trips", "Hours", "FYTD Miles")
...
Private Sub WriteVehicleMonthlyRow(ByVal ws As Worksheet, ByVal rowNumber As Long, ByVal vehicleName As String, ByVal lastOdometer As Variant, ByVal checkoutStatus As String, ByVal monthMiles As Double, ByVal tripDays As Long, ByVal tripCount As Long, ByVal drivingHours As Double, ByVal cumulativeMiles As Double)
    ...
    ws.Cells(rowNumber, 7).value = drivingHours
...
Private Function DrivingHoursForVehicleMonth(ByVal reservationTable As ListObject, ByVal vehicleId As String, ByVal monthStart As Date, ByVal monthEnd As Date) As Double
    Dim i As Long
    Dim rowDate As Variant
    Dim startValue As Variant
    Dim endValue As Variant
        ...
        If StrComp(Trim$(CStr(TableValue(reservationTable, i, "Vehicle ID"))), vehicleId, vbTextCompare) = 0 Then
            ...
            If IsDate(rowDate) Then
                If DateOnly(rowDate) >= monthStart And DateOnly(rowDate) <= monthEnd Then
                    statusText = Trim$(CStr(TableValue(reservationTable, i, "Status")))
                    If StrComp(statusText, "Cancelled", vbTextCompare) <> 0 Then
                        startValue = TableValue(reservationTable, i, "Start Time")
                        endValue = TableValue(reservationTable, i, "End Time")
                        If IsDate(startValue) And IsDate(endValue) Then
                            startTime = CDate(startValue)
                            endTime = CDate(endValue)
                            If endTime < startTime Then endTime = DateAdd("d", 1, endTime)
                            hours = (CDbl(endTime) - CDbl(startTime)) * 24
                            If hours > 0 Then DrivingHoursForVehicleMonth = DrivingHoursForVehicleMonth + hours
                        End If
                    End If
                End If
            End If
...
Private Function BuildMonthlyShopStatusMap(ByVal vehicleTable As ListObject, ByVal reservationTable As ListObject, _
    ...
    EnsureVehicleSupportColumns vehicleTable
    shopEnd = monthEnd
    If shopEnd > Date Then shopEnd = Date
    For vehicleIndex = 1 To vehicleTable.ListRows.Count
        SetDebugStatus "Dashboard: vehicle " & CStr(vehicleIndex)
...
Private Sub DrawWeeklyReservationBlock(ByVal ws As Worksheet, ByVal startRow As Long, ByVal weekStart As Date, ByVal vehicleTable As ListObject, ByVal reservationTable As ListObject)
        ...
        For rowIndex = 1 To UBound(reservationValues, 1)
            ...
            If IsDate(rowDate) Or IsNumeric(rowDate) Then
                ...
                If dayDate >= weekStart And dayDate <= weekEnd Then
                    ...
                    Else
                        entryText = CStr(reservationValues(rowIndex, driverIndex))
    ...
    ws.Rows(CStr(startRow + 4) & ":" & CStr(startRow + 10)).RowHeight = 28
End Sub
...
Public Sub FleetRefreshMonthlyMileageShopStatus(Optional ByVal showMessage As Boolean = False)
    ...
    Dim vehicleStatus As String
    Dim shopStartValue As Variant
    Dim earliestShopDate As Date
    Dim hasShopDate As Boolean
    ...
    For vehicleIndex = 1 To vehicleTable.ListRows.Count
        ...
        If StrComp(vehicleStatus, "Maintenance", vbTextCompare) = 0 Then
            shopStartValue = TableValue(vehicleTable, vehicleIndex, "Shop Start Date")
            If IsDate(shopStartValue) Then
                If Not hasShopDate Or DateOnly(shopStartValue) < earliestShopDate Then
                    earliestShopDate = DateOnly(shopStartValue)
                    hasShopDate = True
                End If
            ElseIf Not hasShopDate Then
                earliestShopDate = Date
                hasShopDate = True
            End If
        End If
    Next vehicleIndex

    If hasShopDate Then RefreshMonthlyMileageShopRange earliestShopDate, Date
...
Public Sub FleetProtectOutputSheets()
    Const sheetPassword As String = "FleetDemo"
    Dim ws As Worksheet
    On Error Resume Next
